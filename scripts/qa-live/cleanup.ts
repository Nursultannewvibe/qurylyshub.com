...
(async () => {
  ...
  const deals = await prisma.deal.findMany({ where: { OR: [{ request: { project_id: { in: pids } } }, { seller_id: { in: cids } }, { buyer_id: { in: uids } }, { product: { name: { startsWith: "[QA]" } } }] }, select: { id: true } });
  ...
  await prisma.product.deleteMany({ where: { name: { startsWith: "[QA]" } } });
