...
export default async function Inbox({ searchParams }: { searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  return <div><h1 className="h1 mb-4">Входящие</h1><Flash sp={sp} />
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="card"><h2 className="h2 mb-2">Коммерческие предложения — КП ({offers.length})</h2>{offers.length ? <table className="table"><thead><tr><th>Заявка</th><th>Компания</th><th>ИТОГО</th><th>Статус</th><th></th></tr></thead><tbody>{offers.map((o) => <tr key={o.id}><td><Link className="text-brand-600" href={`/requests/${o.request_id}`}>{o.request.category.name}</Link><div className="muted">{o.request.project.name}</div></td><td>{o.company.name} <span className="muted">★{o.company.reputation?.avg_rating?.toFixed(1)}</span></td><td><Money v={o.total} /></td><td><Badge s={o.status} /><div className="text-xs text-slate-400"><Dt d={o.created_at} /></div></td><td>{o.status === "sent" && <Link href={`/requests/${o.request_id}`} className="btn-primary text-xs">Сравнить и выбрать</Link>}</td></tr>)}</tbody></table> : <p className="muted">Предложений пока нет. Они появятся, когда поставщики ответят на вашу заявку (обычно в течение дня).</p>}</div>
