...
export default async function NewRequest({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ category?: string; target?: string; ai?: string; delivery_for?: string; error?: string; ok?: string }> }) {
  ...
  const deliveryDeal = sp.delivery_for ? await prisma.deal.findUnique({ where: { id: sp.delivery_for }, include: { seller: true, request: { include: { category: true } }, product: { include: { category: true } } } }) : null;
  const deliveryCatName = deliveryDeal?.request?.category.name ?? deliveryDeal?.product?.category.name ?? "";
  if (deliveryDeal && category.category_type === "logistics") { pre.load_region = deliveryDeal.seller.region ?? ""; pre.load_point = `${deliveryDeal.seller.name}, ${deliveryDeal.seller.city ?? ""}`; pre.unload_region = p.region; pre.unload_point = `${p.name}, ${p.address ?? p.city}`; pre.cargo_type = /бетон/i.test(deliveryCatName) ? "бетон/раствор" : /окн/i.test(deliveryCatName) ? "окна/двери (хрупкое)" : "блоки/кирпич (паллеты)"; if (deliveryDeal.product) pre.weight_t = ""; }
  ...
  return <div className="mx-auto max-w-4xl"><h1 className="h1 mb-1">Заявка: {category.name} <span className="muted">шаблон v{tpl.version}</span></h1><p className="muted mb-4">Объект «{p.name}»{target ? <> · точечный запрос → <b>{target.name}</b></> : " · матчинг по всем подходящим поставщикам"}</p><Flash sp={sp} />
    ...
    {deliveryDeal && <p className="mb-3 rounded bg-brand-50 p-2 text-sm">Доставка для {deliveryDeal.product ? `покупки «${deliveryDeal.product.name}»` : `сделки по «${deliveryCatName}»`} с {deliveryDeal.seller.name}: точки загрузки/выгрузки заполнены — проверьте и при необходимости поправьте. Заявка пройдёт обычный подбор перевозчиков.</p>}
