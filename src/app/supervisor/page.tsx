...
export default async function Supervisor() {
  ...
  const deals = await prisma.deal.findMany({ where: { deal_type: "service_request", status: { in: ["in_progress", "awaiting_payment", "completed"] } }, include: { request: { include: { category: true, project: true } }, seller: true, milestones: true, acts: { where: { act_type: "supervisor_conclusion" } } }, orderBy: { created_at: "desc" } });
