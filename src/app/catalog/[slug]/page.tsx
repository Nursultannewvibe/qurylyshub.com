...
export default async function CompanyPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-3">
    <div className="lg:col-span-2 space-y-4"><Flash sp={sp} />
      ...
      {c.products.length > 0 && <div className="card"><h2 className="h2 mb-1">Товары — купить напрямую</h2><p className="muted mb-2 text-xs">Без заявки и КП: оплата → получение → отзыв. Комиссия платформы включена в цену.</p><table className="table"><thead><tr><th></th><th>Товар</th><th>Цена</th><th>Остаток</th><th>Купить</th></tr></thead><tbody>{c.products.map((p) => <tr key={p.id}><td><ProductGallery photos={(p.photos_json as string[]) ?? []} name={p.name} /></td><td><b>{p.name}</b><div className="muted text-xs">{p.category.name}{p.description ? " · " + p.description : ""}</div></td><td><Money v={p.price} />/{p.unit}</td><td>{p.stock_qty ?? "под заказ"}</td><td>{s && !isOwner ? <form action={buyProductAction} className="flex gap-1"><input type="hidden" name="product_id" value={p.id} /><input type="hidden" name="slug" value={slug} /><input className="input w-20" name="qty" type="number" min={p.min_order_qty} defaultValue={p.min_order_qty} /><button className="btn-primary text-xs" disabled={p.stock_qty === 0}>{p.stock_qty === 0 ? "нет в наличии" : "Купить"}</button></form> : !s ? <Link href="/login" className="text-brand-600 text-xs">войти, чтобы купить</Link> : null}</td></tr>)}</tbody></table></div>}
