...
import { buyProductAction } from "@/server/actions/products";
import { config } from "@/server/config";
import { Money } from "@/components/ui";
import { ProductGallery } from "@/components/product-gallery";
import { addToCompareAction } from "@/server/actions/compare";
...
export default async function CompanyPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  const compareMax = config.compareMax;
  return <div className="grid gap-4 lg:grid-cols-3">
    <div className="lg:col-span-2 space-y-4"><Flash sp={sp} />
      ...
      {c.products.length > 0 && <div className="card"><h2 className="h2 mb-1">Товары — купить напрямую</h2><p className="muted mb-2 text-xs">Без заявки и КП: оплата → получение → отзыв. Комиссия платформы включена в цену.</p><table className="table"><thead><tr><th></th><th>Товар</th><th>Цена</th><th>Остаток</th><th>Купить</th><th>Сравнить</th></tr></thead><tbody>{c.products.map((p) => <tr key={p.id} id={`product-${p.id}`}><td><ProductGallery photos={(p.photos_json as string[]) ?? []} name={p.name} /></td><td><b>{p.name}</b>{p.delivery_days ? <span className="muted text-xs"> · поставка {p.delivery_days} дн.</span> : null}<div className="muted text-xs">{p.category.name}{p.description ? " · " + p.description : ""}</div></td><td><Money v={p.price} />/{p.unit}</td><td>{p.stock_qty ?? "под заказ"}</td><td>{s && !isOwner ? <form action={buyProductAction} className="flex gap-1"><input type="hidden" name="product_id" value={p.id} /><input type="hidden" name="slug" value={slug} /><input className="input w-20" name="qty" type="number" min={p.min_order_qty} defaultValue={p.min_order_qty} /><button className="btn-primary text-xs" disabled={p.stock_qty === 0}>{p.stock_qty === 0 ? "нет в наличии" : "Купить"}</button></form> : !s ? <Link href="/login" className="text-brand-600 text-xs">войти, чтобы купить</Link> : null}</td><td><form action={addToCompareAction}><input type="hidden" name="product_id" value={p.id} /><input type="hidden" name="back" value={`/catalog/${slug}`} /><button className="btn-secondary text-xs" title="Добавить в список сравнения">+ Сравнить</button></form></td></tr>)}</tbody></table><p className="mt-1 text-xs text-slate-400">Сравнение доступно любому вошедшему пользователю: товары одной категории, до {compareMax} позиций.</p></div>}
