...
export default async function CompanyPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-3">
    <div className="lg:col-span-2 space-y-4"><Flash sp={sp} />
      <div className="card"><div className="flex items-start justify-between"><div><h1 className="h1">{c.name}</h1><p className="muted">{c.legal_type.toUpperCase()} · БИН {c.bin} · {c.city ?? c.region} · {c.role === "supplier" ? "поставщик" : "подрядчик"}</p></div><div className="text-right"><div className="text-3xl font-bold">★ {rep?.avg_rating?.toFixed(1) ?? "0.0"}</div><div className="muted">{rep?.deals_count ?? 0} сделок · в срок {rep?.on_time_pct ?? 100}%</div></div></div>
      ...
      {(c.portfolio_json as { title: string; photo: string }[]).length > 0 && <div className="card"><h2 className="h2 mb-2">Портфолио</h2><ul className="text-sm">{(c.portfolio_json as { title: string; photo: string }[]).map((p, i) => <li key={i}>📷 {p.title} <span className="muted">{p.photo}</span></li>)}</ul></div>}
    </div>
