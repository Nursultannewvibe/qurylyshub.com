...
export default async function SupplierProducts({ searchParams }: { searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  return <div><h1 className="h1 mb-1">Товары — {company.name}</h1><p className="muted mb-4">Прямая продажа без заявки и КП: покупатель выбирает товар в вашей <Link className="text-brand-600" href={`/catalog/${company.public_slug}`}>карточке</Link>, оплачивает, подтверждает получение — деньги за вычетом комиссии платформы зачисляются на баланс.</p><Flash sp={sp} />
    <div className="grid gap-4 lg:grid-cols-3"><div className="card lg:col-span-2"><table className="table"><thead><tr><th>Товар</th><th>Категория</th><th>Цена</th><th>Остаток</th><th>Заказов</th><th>Статус</th><th></th></tr></thead><tbody>{products.map((p) => <tr key={p.id}><td><b>{p.name}</b><div className="muted text-xs">{p.description}</div></td><td>{p.category.name}</td><td><Money v={p.price} />/{p.unit}</td><td>{p.stock_qty ?? "под заказ"}</td><td>{p._count.deals}</td><td><Badge s={p.is_active ? "active" : "paused"}>{p.is_active ? "в продаже" : "скрыт"}</Badge></td>
      <td><form action={updateProductAction} className="flex gap-1"><input type="hidden" name="product_id" value={p.id} /><input className="input w-24" name="price" type="number" defaultValue={Number(p.price)} /><input className="input w-20" name="stock_qty" type="number" defaultValue={p.stock_qty ?? ""} placeholder="∞" /><label className="text-xs"><input type="checkbox" name="is_active" defaultChecked={p.is_active} /> в продаже</label><button className="btn-secondary text-xs">Сохранить</button></form></td></tr>)}</tbody></table>{!products.length && <p className="muted">Товаров пока нет — добавьте первый справа.</p>}</div>
