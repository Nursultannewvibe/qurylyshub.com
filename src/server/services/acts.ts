...
export async function generateAct(dealId: string, milestoneId: string | null, type: ActType, actorId: string, extra: { supervisor_id?: string; conclusion?: string } = {}) {
  ...
  const rows = milestone ? milestone.checklist_results.map((r) => `<tr><td>${esc(r.item.item_text)}</td><td>${r.checked ? "✔" : "—"}</td><td>${r.photo_url ? (r.photo_url.startsWith("data:image") ? `<img src="${r.photo_url}" style="max-height:110px;max-width:160px" alt="фото">` : `<a href="${esc(r.photo_url)}">фото</a>`) : r.item.photo_required ? "⚠ нет фото" : ""}</td></tr>`).join("") : "";
